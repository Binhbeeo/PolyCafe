...
<div class="card">
    <table class="table">
        <thead><tr><th>Đồ uống</th><th>Đơn giá</th><th>Số lượng</th><th>Thành tiền</th>
            <% if ("waiting".equals(bill.getStatus())) { %><th>Sửa SL</th><% } %>
        </tr></thead>
        <tbody>
        <%
        ...
        <tfoot>
            <tr>
                <td colspan="<%= "waiting".equals(bill.getStatus()) ? 4 : 3 %>" style="text-align:right"><strong>Tổng cộng:</strong></td>
                <td><strong><%= nf.format(bill.getTotal()) %>đ</strong></td>
                <% if ("waiting".equals(bill.getStatus())) { %><td></td><% } %>
            </tr>
        </tfoot>
    </table>
</div>
